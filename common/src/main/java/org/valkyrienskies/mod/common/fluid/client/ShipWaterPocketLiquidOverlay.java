 ...
 *
 * <p>Closes off the world's water surface where it meets a ship's hull openings, so looking out
 * through a submerged window shows water rather than the hole the hull cuts in the world's own
 * fluid. This is the exterior waterline; interior flood surfaces belong to
 * {@link org.valkyrienskies.mod.common.fluid.FloodedVoxelRenderer}, which understands the streamed
 * partial fill amounts this class predates.</p>
 *
 * <p>Where the surface actually sits comes from {@link ShipExteriorFluidSampler}; which cells sit on
 * the boundary comes from {@link ShipWaterPocketOverlayGeometry}. What is left here is the quad
 * emission itself, drawn through {@link #OVERLAY_RENDER_TYPE} so a shaderpack shades it as water.</p>
 */
public final class ShipWaterPocketLiquidOverlay {
    ...
    public static void clear() {
        ...
        ShipExteriorFluidSampler.clear();
    }
    ...
    /**
     * Emits the overlay quads.
     *
     * <p>Must be called from inside the translucent chunk layer, while a shaderpack still has the
     * terrain-translucent phase active — see {@link #OVERLAY_RENDER_TYPE} for why the call site
     * decides which program these quads end up being drawn with.</p>
     */
    public static void render(final double camX, final double camY, final double camZ) {
        if (!VSGameConfig.CLIENT.getUnderwater().getEnableFluidOverlay()) return;
        ...
        ShipExteriorFluidSampler.ensureExteriorFluidCacheLevel(level);
    ...
    /**
     * Terrain-translucent state, deliberately, so a shaderpack treats these quads as water.
     *
     * <p>Iris picks a program from the render type's shader state and the pipeline's current phase.
     * Drawing with the terrain translucent shader while the phase is
     * {@code TERRAIN_TRANSLUCENT} — which is what {@link #render}'s call site guarantees — resolves
     * to {@code gbuffers_water}, so the pack's own water vertex shader animates and shades this
     * geometry exactly as it does the world's surface. That also fixes the vertex format:
     * {@link DefaultVertexFormat#BLOCK} is what terrain programs declare their attributes against,
     * where the previous entity format would have bound {@code gbuffers_entities} instead.</p>
     */
    private static final RenderType OVERLAY_RENDER_TYPE = new RenderStateShard(null, null, null) {
    ...
    private static int emitOverlayFaces(
        ...
        final ShipWaterPocketOverlayGeometry.Cache cache,
        final ShipFluidRenderSnapshot snapshot,
        final double m00,
        final double m10,
        final double m20,
    ...
    static boolean isOverlaySolidRenderType(final RenderType renderType) {
    ...
    private static @Nullable OverlayFaceSample resolveOverlayFaceSample(final net.minecraft.client.multiplayer.ClientLevel level,
        final ShipFluidRenderSnapshot snapshot, final BlockPos.MutableBlockPos fluidPos,
        ...
        final ShipExteriorFluidSampler.FluidSurfaceSample surface = ShipExteriorFluidSampler.findExteriorFluidSurface(level, fluidPos, scanPos, fluidQueryCache, worldX, worldY, worldZ);
        if (surface == null) {
            return null;
        }
    ...
    private static int emitFaceXRectClipped(final Matrix4f matrix, final VertexConsumer consumer, final float xPlane, final float y0,
        ...
        if (Double.isNaN(fluidSurfaceY) || Math.max(Math.max(wy0, wy1), Math.max(wy2, wy3)) <= fluidSurfaceY + ShipExteriorFluidSampler.SURFACE_EPS) {
            quad(consumer, matrix, sprite, x, y0, z0, x, y0, z1, x, y1, z1, x, y1, z0, u0, v0, u1, v1, normalX, 0.0f, 0.0f,
                r, g, b, a);
            return 1;
        }
        if (Math.min(Math.min(wy0, wy1), Math.min(wy2, wy3)) > fluidSurfaceY + ShipExteriorFluidSampler.SURFACE_EPS) return 0;
    ...
    static boolean touchesFace(final double min, final double max, final boolean towardsPositiveAxis) {
        return towardsPositiveAxis ? max >= 1.0 - ShipExteriorFluidSampler.SURFACE_EPS : min <= ShipExteriorFluidSampler.SURFACE_EPS;
    }
    ...
    private static boolean isSolidAt(final List<AABB> boxes, final double x, final double y, final double z) {
        for (final AABB box : boxes) {
            if (x >= box.minX - ShipExteriorFluidSampler.SURFACE_EPS && x <= box.maxX + ShipExteriorFluidSampler.SURFACE_EPS &&
                y >= box.minY - ShipExteriorFluidSampler.SURFACE_EPS && y <= box.maxY + ShipExteriorFluidSampler.SURFACE_EPS &&
                z >= box.minZ - ShipExteriorFluidSampler.SURFACE_EPS && z <= box.maxZ + ShipExteriorFluidSampler.SURFACE_EPS) {
                return true;
            }
        }
        return false;
    ...
    static @Nullable AABB getOverlayShapeBounds(final net.minecraft.client.multiplayer.ClientLevel level, final BlockPos pos) {
        final BlockState state = level.getBlockState(pos);
        final VoxelShape shape = resolveOverlayShape(level, pos, state);
        if (shape.isEmpty()) {
            return null;
        ...
        if (bounds.getXsize() <= ShipExteriorFluidSampler.SURFACE_EPS || bounds.getYsize() <= ShipExteriorFluidSampler.SURFACE_EPS || bounds.getZsize() <= ShipExteriorFluidSampler.SURFACE_EPS) {
            return null;
        }
        return bounds;
    }
    ...
    interface OverlayFaceVisitor {
        void faceX(float xPlane, float y0, float y1, float z0, float z1, float normalX, double fluidSurfaceY);
        void faceY(float yPlane, float x0, float x1, float z0, float z1, float normalY, double fluidSurfaceY);
        void faceZ(float zPlane, float x0, float x1, float y0, float y1, float normalZ, double fluidSurfaceY);
    }

    static void visitOverlayFaceRects(final ClientLevel level, final int sizeX, final int sizeY, final int sizeZ,
        final int shipMinX, final int shipMinY, final int shipMinZ,
        final BitSet open, final BitSet interior, final @Nullable BitSet overlaySolids,
        final @Nullable BitSet fullCellOverlaySolids, final @Nullable BitSet overlayBoundary,
        ...
        final OverlayFaceVisitor visitor) {
        if (overlayBoundary == null || overlayBoundary.isEmpty()) return;
        ...
        for (int outsideIdx = overlayBoundary.nextSetBit(0); outsideIdx >= 0 && outsideIdx < volume;
             outsideIdx = overlayBoundary.nextSetBit(outsideIdx + 1)) {
            final int lx = outsideIdx % sizeX;
            final int t = outsideIdx / sizeX;
            final int ly = t % sizeY;
            final int lz = t / sizeY;

            // Same "stableSample" reuse emitOverlayFaces does: the cell-center sample is tried
            // first and shared across all faces of this outside cell; only recompute per-face if
            // the shared sample failed (e.g. right at the shipyard boundary).
            final double stableSurfaceY = ShipExteriorFluidSampler.sampleSurfaceY(level, fluidPos, scanPos, fluidQueryCache,
                lx + 0.5, ly + 0.5, lz + 0.5, m00, m10, m20, m01, m11, m21, m02, m12, m22, tX, tY, tZ);

            if (lx > 0) {
                final int n = outsideIdx - 1;
                if (isInteriorOpen(open, interior, n)) {
                    final double sy = Double.isNaN(stableSurfaceY) ? ShipExteriorFluidSampler.sampleSurfaceY(level, fluidPos, scanPos,
                        fluidQueryCache, lx + FACE_SAMPLE_OFFSET, ly + 0.5, lz + 0.5, m00, m10, m20, m01, m11, m21, m02,
                        m12, m22, tX, tY, tZ) : stableSurfaceY;
                    if (!Double.isNaN(sy)) visitor.faceX(lx, ly, ly + 1.0f, lz, lz + 1.0f, +1.0f, sy);
                } else if (isFullCellOverlaySolid(fullCellOverlaySolids, n)
                    || (!open.get(n) && overlaySolids != null && overlaySolids.get(n))) {
                    final double sy = Double.isNaN(stableSurfaceY) ? ShipExteriorFluidSampler.sampleSurfaceY(level, fluidPos, scanPos,
                        fluidQueryCache, lx + FACE_SAMPLE_OFFSET, ly + 0.5, lz + 0.5, m00, m10, m20, m01, m11, m21, m02,
                        m12, m22, tX, tY, tZ) : stableSurfaceY;
                    if (!Double.isNaN(sy)) {
                        visitSolidFaceRectX(level, solidPos, shipMinX, shipMinY, shipMinZ, lx - 1, ly, lz, true, +1.0f, sy, visitor);
                    }
                }
            }
            if (lx + 1 < sizeX) {
                ...
                if (isInteriorOpen(open, interior, n)) {
                    final double sy = Double.isNaN(stableSurfaceY) ? ShipExteriorFluidSampler.sampleSurfaceY(level, fluidPos, scanPos,
                        fluidQueryCache, lx + 1.0 - FACE_SAMPLE_OFFSET, ly + 0.5, lz + 0.5, m00, m10, m20, m01, m11, m21,
                        m02, m12, m22, tX, tY, tZ) : stableSurfaceY;
                    if (!Double.isNaN(sy)) visitor.faceX(lx + 1, ly, ly + 1.0f, lz, lz + 1.0f, -1.0f, sy);
                } else if (isFullCellOverlaySolid(fullCellOverlaySolids, n)
                    || (!open.get(n) && overlaySolids != null && overlaySolids.get(n))) {
                    final double sy = Double.isNaN(stableSurfaceY) ? ShipExteriorFluidSampler.sampleSurfaceY(level, fluidPos, scanPos,
                        fluidQueryCache, lx + 1.0 - FACE_SAMPLE_OFFSET, ly + 0.5, lz + 0.5, m00, m10, m20, m01, m11, m21,
                        m02, m12, m22, tX, tY, tZ) : stableSurfaceY;
                    if (!Double.isNaN(sy)) {
                        visitSolidFaceRectX(level, solidPos, shipMinX, shipMinY, shipMinZ, lx + 1, ly, lz, false, -1.0f, sy, visitor);
                    }
                }
            }
            if (ly > 0) {
                ...
                if (isInteriorOpen(open, interior, n)) {
                    final double sy = Double.isNaN(stableSurfaceY) ? ShipExteriorFluidSampler.sampleSurfaceY(level, fluidPos, scanPos,
                        fluidQueryCache, lx + 0.5, ly + FACE_SAMPLE_OFFSET, lz + 0.5, m00, m10, m20, m01, m11, m21, m02,
                        m12, m22, tX, tY, tZ) : stableSurfaceY;
                    if (!Double.isNaN(sy)) visitor.faceY(ly, lx, lx + 1.0f, lz, lz + 1.0f, +1.0f, sy);
                } else if (isFullCellOverlaySolid(fullCellOverlaySolids, n)
                    || (!open.get(n) && overlaySolids != null && overlaySolids.get(n))) {
                    final double sy = Double.isNaN(stableSurfaceY) ? ShipExteriorFluidSampler.sampleSurfaceY(level, fluidPos, scanPos,
                        fluidQueryCache, lx + 0.5, ly + FACE_SAMPLE_OFFSET, lz + 0.5, m00, m10, m20, m01, m11, m21, m02,
                        m12, m22, tX, tY, tZ) : stableSurfaceY;
                    if (!Double.isNaN(sy)) {
                        visitSolidFaceRectY(level, solidPos, shipMinX, shipMinY, shipMinZ, lx, ly - 1, lz, true, +1.0f, sy, visitor);
                    }
                }
            }
            if (ly + 1 < sizeY) {
                ...
                if (isInteriorOpen(open, interior, n)) {
                    final double sy = Double.isNaN(stableSurfaceY) ? ShipExteriorFluidSampler.sampleSurfaceY(level, fluidPos, scanPos,
                        fluidQueryCache, lx + 0.5, ly + 1.0 - FACE_SAMPLE_OFFSET, lz + 0.5, m00, m10, m20, m01, m11, m21,
                        m02, m12, m22, tX, tY, tZ) : stableSurfaceY;
                    if (!Double.isNaN(sy)) visitor.faceY(ly + 1, lx, lx + 1.0f, lz, lz + 1.0f, -1.0f, sy);
                } else if (isFullCellOverlaySolid(fullCellOverlaySolids, n)
                    || (!open.get(n) && overlaySolids != null && overlaySolids.get(n))) {
                    final double sy = Double.isNaN(stableSurfaceY) ? ShipExteriorFluidSampler.sampleSurfaceY(level, fluidPos, scanPos,
                        fluidQueryCache, lx + 0.5, ly + 1.0 - FACE_SAMPLE_OFFSET, lz + 0.5, m00, m10, m20, m01, m11, m21,
                        m02, m12, m22, tX, tY, tZ) : stableSurfaceY;
                    if (!Double.isNaN(sy)) {
                        visitSolidFaceRectY(level, solidPos, shipMinX, shipMinY, shipMinZ, lx, ly + 1, lz, false, -1.0f, sy, visitor);
                    }
                }
            }
            if (lz > 0) {
                ...
                if (isInteriorOpen(open, interior, n)) {
                    final double sy = Double.isNaN(stableSurfaceY) ? ShipExteriorFluidSampler.sampleSurfaceY(level, fluidPos, scanPos,
                        fluidQueryCache, lx + 0.5, ly + 0.5, lz + FACE_SAMPLE_OFFSET, m00, m10, m20, m01, m11, m21, m02,
                        m12, m22, tX, tY, tZ) : stableSurfaceY;
                    if (!Double.isNaN(sy)) visitor.faceZ(lz, lx, lx + 1.0f, ly, ly + 1.0f, +1.0f, sy);
                } else if (isFullCellOverlaySolid(fullCellOverlaySolids, n)
                    || (!open.get(n) && overlaySolids != null && overlaySolids.get(n))) {
                    final double sy = Double.isNaN(stableSurfaceY) ? ShipExteriorFluidSampler.sampleSurfaceY(level, fluidPos, scanPos,
                        fluidQueryCache, lx + 0.5, ly + 0.5, lz + FACE_SAMPLE_OFFSET, m00, m10, m20, m01, m11, m21, m02,
                        m12, m22, tX, tY, tZ) : stableSurfaceY;
                    if (!Double.isNaN(sy)) {
                        visitSolidFaceRectZ(level, solidPos, shipMinX, shipMinY, shipMinZ, lx, ly, lz - 1, true, +1.0f, sy, visitor);
                    }
                }
            }
            if (lz + 1 < sizeZ) {
                ...
                if (isInteriorOpen(open, interior, n)) {
                    final double sy = Double.isNaN(stableSurfaceY) ? ShipExteriorFluidSampler.sampleSurfaceY(level, fluidPos, scanPos,
                        fluidQueryCache, lx + 0.5, ly + 0.5, lz + 1.0 - FACE_SAMPLE_OFFSET, m00, m10, m20, m01, m11, m21,
                        m02, m12, m22, tX, tY, tZ) : stableSurfaceY;
                    if (!Double.isNaN(sy)) visitor.faceZ(lz + 1, lx, lx + 1.0f, ly, ly + 1.0f, -1.0f, sy);
                } else if (isFullCellOverlaySolid(fullCellOverlaySolids, n)
                    || (!open.get(n) && overlaySolids != null && overlaySolids.get(n))) {
                    final double sy = Double.isNaN(stableSurfaceY) ? ShipExteriorFluidSampler.sampleSurfaceY(level, fluidPos, scanPos,
                        fluidQueryCache, lx + 0.5, ly + 0.5, lz + 1.0 - FACE_SAMPLE_OFFSET, m00, m10, m20, m01, m11, m21,
                        m02, m12, m22, tX, tY, tZ) : stableSurfaceY;
                    if (!Double.isNaN(sy)) {
                        visitSolidFaceRectZ(level, solidPos, shipMinX, shipMinY, shipMinZ, lx, ly, lz + 1, false, -1.0f, sy, visitor);
                    }
                }
            }
        }
    ...
    static int clipToSurfaceY(final float[] inX, final float[] inY, final float[] inZ, final float[] inU, final float[] inV,
        ...
        boolean prevInside = prevWorldY <= fluidSurfaceY + ShipExteriorFluidSampler.SURFACE_EPS;
        ...
        for (int i = 0; i < inCount; i++) {
            ...
            final boolean curInside = curWorldY <= fluidSurfaceY + ShipExteriorFluidSampler.SURFACE_EPS;
    ...
    private static void visitSolidFaceRectX(final ClientLevel level, final BlockPos.MutableBlockPos pos,
        final int shipMinX, final int shipMinY, final int shipMinZ, final int solidX, final int solidY, final int solidZ,
        final boolean towardsPositiveAxis, final float normalX, final double fluidSurfaceY, final OverlayFaceVisitor visitor) {
        pos.set(shipMinX + solidX, shipMinY + solidY, shipMinZ + solidZ);
        final BlockState state = level.getBlockState(pos);
        if (shouldUseFullCellSolidOverlay(state)) {
            final float xPlane = towardsPositiveAxis ? solidX + 1.0f : solidX;
            visitor.faceX(xPlane, solidY, solidY + 1.0f, solidZ, solidZ + 1.0f, normalX, fluidSurfaceY);
            return;
        }
        final AABB bounds = getOverlayShapeBounds(level, pos);
        if (bounds == null || !touchesFace(bounds.minX, bounds.maxX, towardsPositiveAxis)) return;
        final float xPlane = solidX + (towardsPositiveAxis ? (float) bounds.maxX : (float) bounds.minX);
        visitor.faceX(xPlane, solidY + (float) bounds.minY, solidY + (float) bounds.maxY,
            solidZ + (float) bounds.minZ, solidZ + (float) bounds.maxZ, normalX, fluidSurfaceY);
    }
    // visitSolidFaceRectY / Z: same pattern

    private static void visitSolidFaceRectY(final net.minecraft.client.multiplayer.ClientLevel level,
        final BlockPos.MutableBlockPos pos, final int shipMinX, final int shipMinY, final int shipMinZ,
        final int solidX, final int solidY, final int solidZ, final boolean towardsPositiveAxis, final float normalY,
        final double fluidSurfaceY, final OverlayFaceVisitor visitor) {
        pos.set(shipMinX + solidX, shipMinY + solidY, shipMinZ + solidZ);
        final BlockState state = level.getBlockState(pos);
        if (shouldUseFullCellSolidOverlay(state)) {
            final float yPlane = towardsPositiveAxis ? solidY + 1.0f : solidY;
            visitor.faceY(yPlane, solidX, solidX + 1.0f, solidZ, solidZ + 1.0f, normalY, fluidSurfaceY);
            return;
        }
        final AABB bounds = getOverlayShapeBounds(level, pos);
        if (bounds == null || !touchesFace(bounds.minY, bounds.maxY, towardsPositiveAxis)) return;
        final float yPlane = solidY + (towardsPositiveAxis ? (float) bounds.maxY : (float) bounds.minY);
        visitor.faceY(yPlane, solidX + (float) bounds.minX, solidX + (float) bounds.maxX,
            solidZ + (float) bounds.minZ, solidZ + (float) bounds.maxZ, normalY, fluidSurfaceY);
    }

    private static void visitSolidFaceRectZ(final net.minecraft.client.multiplayer.ClientLevel level,
        final BlockPos.MutableBlockPos pos, final int shipMinX, final int shipMinY, final int shipMinZ,
        final int solidX, final int solidY, final int solidZ, final boolean towardsPositiveAxis, final float normalZ,
        final double fluidSurfaceY, final OverlayFaceVisitor visitor) {
        pos.set(shipMinX + solidX, shipMinY + solidY, shipMinZ + solidZ);
        final BlockState state = level.getBlockState(pos);
        if (shouldUseFullCellSolidOverlay(state)) {
            final float zPlane = towardsPositiveAxis ? solidZ + 1.0f : solidZ;
            visitor.faceZ(zPlane, solidX, solidX + 1.0f, solidY, solidY + 1.0f, normalZ, fluidSurfaceY);
            return;
        }
        final AABB bounds = getOverlayShapeBounds(level, pos);
        if (bounds == null || !touchesFace(bounds.minZ, bounds.maxZ, towardsPositiveAxis)) return;
        final float zPlane = solidZ + (towardsPositiveAxis ? (float) bounds.maxZ : (float) bounds.minZ);
        visitor.faceZ(zPlane, solidX + (float) bounds.minX, solidX + (float) bounds.maxX,
            solidY + (float) bounds.minY, solidY + (float) bounds.maxY, normalZ, fluidSurfaceY);
    }
    ...
    private static void vertex(final VertexConsumer consumer, final Matrix4f matrix, final TextureAtlasSprite sprite, final float x,
        ...
        // DefaultVertexFormat.BLOCK: no overlay coords, unlike the entity format this used to use.
