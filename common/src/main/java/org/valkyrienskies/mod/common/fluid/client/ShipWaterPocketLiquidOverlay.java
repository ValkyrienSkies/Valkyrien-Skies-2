 ...
 *
 * <p><b>Only the sampling half of this class is live.</b> Interior flood surfaces are drawn by
 * {@link org.valkyrienskies.mod.common.fluid.FloodedVoxelRenderer}, which understands the streamed
 * partial fill amounts this overlay predates, so {@link #render} is deliberately not wired into the
 * render pipeline. What is still used is the exterior-water surface solver — the vanilla-faithful
 * column heights, their chunk-revision caches, and the boundary-mask helpers — which
 * {@link ShipPocketWorldWaterOccluder} and {@link ShipWaterPocketOverlayGeometry} depend on to know
 * where the waterline actually falls. Splitting that out is worthwhile follow-up; it is left intact
 * here so the surface math stays byte-for-byte what was tested.</p>
 */
public final class ShipWaterPocketLiquidOverlay {
    ...
    static final double SURFACE_EPS = 1.0E-5;
    ...
    static Fluid canonicalSource(final Fluid fluid) {
     ...
     */
    private static ColumnHeights computeVanillaColumnHeights(final net.minecraft.client.multiplayer.ClientLevel level,
    ...
    private static float vanillaCornerHeight(final net.minecraft.client.multiplayer.ClientLevel level,
     ...
     */
    private static float vanillaFluidHeight(final net.minecraft.client.multiplayer.ClientLevel level,
        final FluidStateManager.QueryCache fluidQueryCache, final BlockPos pos, final Fluid canonicalFluid) {
    ...
    static void visitOverlayFaceRects(final ClientLevel level, final int sizeX, final int sizeY, final int sizeZ,
        ...
        for (int outsideIdx = overlayBoundary.nextSetBit(0); outsideIdx >= 0 && outsideIdx < volume;
            ...
            final double stableSurfaceY = sampleOverlayFaceSurfaceY(level, fluidPos, scanPos, fluidQueryCache,
                lx + 0.5, ly + 0.5, lz + 0.5, m00, m10, m20, m01, m11, m21, m02, m12, m22, tX, tY, tZ);

            if (lx > 0) {
                final int n = outsideIdx - 1;
                if (isInteriorOpen(open, interior, n)) {
                    final double sy = Double.isNaN(stableSurfaceY) ? sampleOverlayFaceSurfaceY(level, fluidPos, scanPos,
    ...
    static double sampleOverlayFaceSurfaceY(final ClientLevel level, final BlockPos.MutableBlockPos fluidPos,
